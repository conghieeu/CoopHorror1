...
public class RoundManager : NetworkBehaviour
{
	...
	public void SpawnScrapInLevel()
	{
		...
		for (int i = 0; i < currentLevel.spawnableScrap.Count; i++)
		{
			...
			{
				list2.Add(currentLevel.spawnableScrap[i].rarity);
			}
		}
		...
		int k;
		for (k = 0; k < ScrapToSpawn.Count; k++)
		{
			if (ScrapToSpawn[k] == null)
			{
				Debug.Log("Error!!!!! Found null element in list ScrapToSpawn. Skipping it.");
				continue;
			}
			List<RandomScrapSpawn> list4 = ((ScrapToSpawn[k].spawnPositionTypes != null && ScrapToSpawn[k].spawnPositionTypes.Count != 0) ? source.Where((RandomScrapSpawn x) => ScrapToSpawn[k].spawnPositionTypes.Contains(x.spawnableItems) && !x.spawnUsed).ToList() : source.ToList());
			if (list4.Count <= 0)
			{
				Debug.Log("No tiles containing a scrap spawn with item type: " + ScrapToSpawn[k].itemName);
				continue;
			}
			...
			{
				position = GetRandomNavMeshPositionInBoxPredictable(randomScrapSpawn.transform.position, randomScrapSpawn.itemSpawnRange, navHit, AnomalyRandom) + Vector3.up * ScrapToSpawn[k].verticalOffset;
			}
			GameObject obj = UnityEngine.Object.Instantiate(ScrapToSpawn[k].spawnPrefab, position, Quaternion.identity, spawnedScrapContainer);
			GrabbableObject component = obj.GetComponent<GrabbableObject>();
			component.transform.rotation = Quaternion.Euler(component.itemProperties.restingRotation);
			component.fallTime = 0f;
			list.Add((int)((float)AnomalyRandom.Next(ScrapToSpawn[k].minValue, ScrapToSpawn[k].maxValue) * scrapValueMultiplier));
	...
	private GameObject SpawnRandomDaytimeEnemy(GameObject[] spawnPoints, float timeUpToCurrentHour)
	{
		...
		for (int i = 0; i < currentLevel.DaytimeEnemies.Count; i++)
		{
			...
			if (enemyType.PowerLevel > currentLevel.maxDaytimeEnemyPowerCount - currentDaytimeEnemyPower || enemyType.numberSpawned >= currentLevel.DaytimeEnemies[i].enemyType.MaxCount || enemyType.normalizedTimeInDayToLeave < TimeOfDay.Instance.normalizedTimeOfDay || enemyType.spawningDisabled)
			{
		...
		Debug.Log($"Anomaly random 3: {position.x}, {position.y}, {position.z}");
		int num3 = 0;
		bool flag = false;
	...
	private GameObject SpawnRandomOutsideEnemy(GameObject[] spawnPoints, float timeUpToCurrentHour)
	{
		...
		for (int j = 0; j < spawnPoints.Length - 1; j++)
		{
			...
			{
				flag = true;
				if (Vector3.Distance(position, spawnDenialPoints[k].transform.position) < 16f)
				{
					num3 = (num3 + 1) % spawnPoints.Length;
					position = spawnPoints[num3].transform.position;
					position = GetRandomNavMeshPositionInRadius(position, 4f);
	...
	private void SpawnOutsideHazards()
	{
		...
		if (TimeOfDay.Instance.currentLevelWeather == LevelWeatherType.Rainy)
		{
			...
			for (int num2 = 0; num2 < num; num2++)
			{
				...
				Vector3 position2 = GetRandomNavMeshPositionInBoxPredictable(position, 30f, hit, random) + Vector3.up;
		...
		{
			for (int num4 = 0; num4 < currentLevel.spawnableOutsideObjects.Length; num4++)
			{
				double num5 = random.NextDouble();
				num = (int)currentLevel.spawnableOutsideObjects[num4].randomAmount.Evaluate((float)num5);
				if (increasedMapPropSpawnRateIndex == num4)
				{
					num += 12;
				}
				...
				for (int num6 = 0; num6 < num; num6++)
				{
					...
					Vector3 position3 = outsideAINodes[num7].transform.position;
					Vector3 position2 = GetRandomNavMeshPositionInBoxPredictable(position3, 30f, hit, random);
					if (currentLevel.spawnableOutsideObjects[num4].spawnableObject.spawnableFloorTags != null)
					{
						bool flag = false;
						...
						{
							for (int num8 = 0; num8 < currentLevel.spawnableOutsideObjects[num4].spawnableObject.spawnableFloorTags.Length; num8++)
							{
								if (hitInfo.collider.transform.CompareTag(currentLevel.spawnableOutsideObjects[num4].spawnableObject.spawnableFloorTags[num8]))
								{
									flag = true;
									break;
								}
					...
					if (NavMesh.FindClosestEdge(position2, out hit, -1) && hit.distance < (float)currentLevel.spawnableOutsideObjects[num4].spawnableObject.objectWidth)
					{
						Vector3 position4 = hit.position;
						if (!NavMesh.SamplePosition(new Ray(position4, position2 - position4).GetPoint((float)currentLevel.spawnableOutsideObjects[num4].spawnableObject.objectWidth + 0.5f), out hit, 10f, -1))
						{
							continue;
						}
					...
					bool flag2 = false;
					for (int num9 = 0; num9 < shipSpawnPathPoints.Length; num9++)
					{
						if (Vector3.Distance(shipSpawnPathPoints[num9].transform.position, position2) < (float)currentLevel.spawnableOutsideObjects[num4].spawnableObject.objectWidth + 6f)
						{
							flag2 = true;
							break;
						}
					...
					{
						if (Vector3.Distance(spawnDenialPoints[num10].transform.position, position2) < (float)currentLevel.spawnableOutsideObjects[num4].spawnableObject.objectWidth + 6f)
						{
							flag2 = true;
							break;
						}
					}
					if (Vector3.Distance(GameObject.FindGameObjectWithTag("ItemShipLandingNode").transform.position, position2) < (float)currentLevel.spawnableOutsideObjects[num4].spawnableObject.objectWidth + 4f)
					{
						flag2 = true;
						break;
					}
					...
					if (currentLevel.spawnableOutsideObjects[num4].spawnableObject.objectWidth > 4)
					{
						flag2 = false;
						for (int num11 = 0; num11 < list.Count; num11++)
						{
							if (Vector3.Distance(position2, list[num11]) < (float)currentLevel.spawnableOutsideObjects[num4].spawnableObject.objectWidth)
							{
								flag2 = true;
								break;
							}
					...
					list.Add(position2);
					GameObject gameObject = UnityEngine.Object.Instantiate(currentLevel.spawnableOutsideObjects[num4].spawnableObject.prefabToSpawn, position2 - Vector3.up * 0.7f, Quaternion.identity, mapPropsContainer.transform);
					num3++;
					if (currentLevel.spawnableOutsideObjects[num4].spawnableObject.spawnFacingAwayFromWall)
					{
					...
					else
					{
						...
						gameObject.transform.eulerAngles = new Vector3(gameObject.transform.eulerAngles.x, num12, gameObject.transform.eulerAngles.z);
					}
					gameObject.transform.localEulerAngles = new Vector3(gameObject.transform.localEulerAngles.x + currentLevel.spawnableOutsideObjects[num4].spawnableObject.rotationOffset.x, gameObject.transform.localEulerAngles.y + currentLevel.spawnableOutsideObjects[num4].spawnableObject.rotationOffset.y, gameObject.transform.localEulerAngles.z + currentLevel.spawnableOutsideObjects[num4].spawnableObject.rotationOffset.z);
				}
			}
		}
		if (num3 > 0)
	...
	private void SetLockedDoors(Vector3 mainEntrancePosition)
	{
		...
		if (base.IsServer)
		{
			...
			{
				int num6 = AnomalyRandom.Next(0, insideAINodes.Length);
				Vector3 randomNavMeshPositionInBoxPredictable = GetRandomNavMeshPositionInBoxPredictable(insideAINodes[num6].transform.position, 8f, navHit, AnomalyRandom);
