...
namespace GameNetcodeStuff
{
	public class PlayerControllerB : NetworkBehaviour, IHittable, IShockableWithGun, IVisibleThreat
	{
		...
		private void OpenMenu_performed(InputAction.CallbackContext context)
		{
			...
			Debug.Log($"In terminal menu: {inTerminalMenu}");
			if (!context.performed)
			{
				return;
			}
		...
		public void Crouch(bool crouch)
		{
			...
			else if (!Physics.Raycast(gameplayCamera.transform.position, Vector3.up, out hit, 0.8f, playersManager.collidersAndRoomMask, QueryTriggerInteraction.Ignore))
			{
				isCrouching = false;
				StartOfRound.Instance.timeAtMakingLastPersonalMovement = Time.realtimeSinceStartup;
		...
		private void PlayerHitGroundEffects()
		{
			...
			if (takingFallDamage && !jetpackControls && !disablingJetpackControls && !isSpeedCheating)
			{
				Debug.Log($"Fall damage: {fallValueUncapped}");
		...
		public void SpawnDeadBody(int playerId, Vector3 bodyVelocity, int causeOfDeath, PlayerControllerB deadPlayerController, int deathAnimation = 0, Transform overridePosition = null)
		{
			...
			GameObject gameObject = ((!(overridePosition != null)) ? UnityEngine.Object.Instantiate(playersManager.playerRagdolls[deathAnimation], deadPlayerController.thisPlayerBody.position + Vector3.up * num, deadPlayerController.thisPlayerBody.rotation, parent) : UnityEngine.Object.Instantiate(playersManager.playerRagdolls[deathAnimation], overridePosition.position + Vector3.up * num, overridePosition.rotation, parent));
