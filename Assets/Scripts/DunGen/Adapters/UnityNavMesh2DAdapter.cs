		...
		protected virtual List<NavMeshBuildSource> CollectSources()
		{
			...
			foreach (NavMeshModifier item2 in list2)
			{
				if (((int)layerMask & (1 << item2.gameObject.layer)) != 0 && item2.AffectsAgentType(agentTypeID))
				{
					...
					{
						root = item2.transform,
						overrideArea = item2.overrideArea,
						area = item2.area,
						ignoreFromBuild = item2.ignoreFromBuild
					});
				}
			}
			...
			{
				for (int num2 = tilemap.cellBounds.xMin; num2 < tilemap.cellBounds.xMax; num2++)
				{
					for (int num3 = tilemap.cellBounds.yMin; num3 < tilemap.cellBounds.yMax; num3++)
					{
						Vector3Int position = new Vector3Int(num2, num3, 0);
						if (tilemap.HasTile(position))
						{
							...
							if (mesh2 != null)
							{
								item.transform = Matrix4x4.TRS(tilemap.GetCellCenterWorld(position) - tilemap.layoutGrid.cellGap, tilemap.transform.rotation, tilemap.transform.lossyScale) * tilemap.orientationMatrix * tilemap.GetTransformMatrix(position);
								item.sourceObject = mesh2;
								item.component = tilemap;
								item.area = ((tile.colliderType != UnityEngine.Tilemaps.Tile.ColliderType.None) ? unwalkableArea : 0);
								sources.Add(item);
							}
						}
					}
