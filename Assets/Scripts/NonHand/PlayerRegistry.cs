...
namespace NonHand
{
	...
	public class PlayerRegistry : NetworkBehaviour, INetworkRunnerCallbacks
	{
		...
		/// <summary>
		/// Kiểm tra có player nào thỏa mãn điều kiện không. Return ngay khi tìm thấy match đầu tiên.
		/// Nếu thiếu hàm này, phải dùng CountWhere() hoặc Where().Any() - kém hiệu quả vì vẫn duyệt hết danh sách dù đã tìm thấy.
		/// </summary>
